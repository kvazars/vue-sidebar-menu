...
export default function useItem(props, emits) {
	...
	const onMouseLeave = () => {
		...
		if (isMobileItem.value) {
			unsetMobileItem(
				false,
				!sidebarProps.disableHover ? 300 : undefined
			);
		}
	};
		...
		if (isCollapsed.value) {
			setTimeout(() => {
				...
				if (event.type === "click" && !hasChild.value) {
					unsetMobileItem(
						false,
						!isFirstLevel.value ? 300 : undefined
					);
				}
			}, 0);
		}
	...
	const isHidden = computed(() => {
		if (isCollapsed.value) {
			if (
				props.item.hidden &&
				props.item.hiddenOnCollapse === undefined
			) {
				return true;
			} else {
	...
	const mobileItemDropdownStyle = computed(() => {
		return {
			...
			...(!mobileItemRect.value.dropup
				? {
						top: `${
							mobileItemRect.value.top +
							mobileItemRect.value.height
						}px`,
				  }
